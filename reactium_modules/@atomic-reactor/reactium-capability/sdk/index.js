...
const update = async (capability, role, action) => {
    ...
    const result = await SDK.API.Actinium.Cloud.run(action, {
        capability,
        role,
    });
...
Capability.propagate = async (force = false) => {
    ...
    req =
        capabilities.length > 0
            ? SDK.API.Actinium.Cloud.run('capability-create-all', {
                  capabilities,
              })
            : Promise.resolve();
    ...
    return req
        ...
        .catch(err => {
            ...
            if (
                ...
                retry += 1;
                req = SDK.API.Actinium.Cloud.run('capability-create-all', {
                    capabilities,
                });
                req.retry = retry;
                req.propagate = true;
...
Capability.User.get = async (user, refresh = false) => {
    ...
    if (!caps || refresh === true) {
        ...
        if (!req) {
            req = SDK.API.Actinium.Cloud.run('capability-get-user', { user })
                .then(caps => {
                    Cache.set(cacheKey, caps, Capability.cache);
