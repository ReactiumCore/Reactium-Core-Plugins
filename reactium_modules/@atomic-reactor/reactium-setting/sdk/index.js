...
Setting.load = async (autoRefresh = true) => {
    ...
    const settings = await SDK.Cloud.run('settings');
...
Setting.set = async (key = '', value, setPublic = false) => {
    ...
    const setting = await SDK.Cloud.run('setting-set', {
        key,
        value,
        public: setPublic,
    });
...
Setting.unset = async (key = '') => {
    const setting = await SDK.Cloud.run('setting-unset', { key });
...
Setting.get = async (key = '', defaultValue, refresh = false) => {
    ...
    const value = await SDK.Cloud.run('setting-get', { key: group });
    Cache.set(['setting', group], value, Enums.cache.settings);
