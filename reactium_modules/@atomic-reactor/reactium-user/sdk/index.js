...
User.logOut = async () => {
    ...
    try {
        return SDK.API.Actinium.User.logOut().then(async () => {
 ...
 */
User.forgot = email =>
    SDK.API.Actinium.Cloud.run('password-reset-request', { email });
...
User.reset = (token, password) =>
    SDK.API.Actinium.Cloud.run('password-reset', { token, password });
...
User.hasValidSession = async () => {
    ...
    request = SDK.API.Actinium.Cloud.run('session-validate')
        .then(() => Promise.resolve(true))
        .catch(error => {
            // destroy current user on invalid session
            if (op.get(error, 'code') === 209) {
                SDK.API.Actinium.CoreManager.getUserController().removeUserFromDisk();
...
User.register = async user => {
    ...
    let response = await SDK.API.Actinium.Cloud.run('user-save', user);
...
User.list = async params => {
    ...
    let response = await SDK.API.Actinium.Cloud.run('user-list', params);
...
User.save = async params => {
    ...
    return SDK.API.Actinium.Cloud.run('user-save', params)
        .then(async response => {
...
User.trash = async objectId => {
    ...
    await SDK.API.Actinium.Cloud.run('user-trash', { objectId });
...
User.retrieve = async params => {
    ...
    req = new Promise(async resolve => {
        const response = await SDK.API.Actinium.Cloud.run(
            'user-retrieve',
            params,
        );
...
User.Role.add = async (role, objectId) => {
    ...
    return SDK.API.Actinium.Cloud.run('role-user-add', { role, user: u })
...
User.Role.remove = (role, objectId) => {
    ...
    return SDK.API.Actinium.Cloud.run('role-user-remove', { role, user: u })
        .then(() => User.retrieve({ objectId: u, refresh: true }))
        .then(async u => {
